...
public class PathFinder {
	...
	public PathFinder(Maze maze) {
		...
		reversedPath = new LinkedStack<Position>();
		this.maze = maze;
	}
	...
	public StackADT<Position> findPath() {
		...
		while (!found && !reversedPath.isEmpty()) {
			 ...
			 */
			Position current = reversedPath.peek(); 
			...
			if (current.equals(maze.exit())) {
				...
				List<Position> validMoves = maze.potentialMovesFrom(current);
				
				if (validMoves.isEmpty()) {
					reversedPath.pop();
				}else {
					Position next = validMoves.get(0);
