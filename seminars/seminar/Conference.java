...
public class Conference {
	...
	public Conference(String file)
		...
		while(in.hasNextLine()){
			 ...
			 */
			int speakertype;
			if(attributes.length < 4 || attributes.length > 4 ){
				throw new BadDataFormatException();
			}
			else{
				try {
					speakertype = Integer.parseInt(attributes[3]);
				} catch (NumberFormatException nfe) {
					throw nfe;
				}
				Seminar sem = new Seminar(attributes[0], attributes[1], attributes[2], speakertype);
				talks.add(sem);
			}
